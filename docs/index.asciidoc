...
Deletes the DLQ segments that has been read, when it's configured.
This feature implicitly requires `commit_offsets` set to `true`, if it's not then result in a configuration error.
It's supported only for Logstash >= 8.4.0. If this setting is `true` and Logstash version doesn't provides this feature then result in a configuration error.
